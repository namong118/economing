...
절대 금지:
- 특정 투자 종목 추천 (삼성전자, 애플 같은 개별 종목)
- 수익률 예측 ("연 X% 수익" 같은 표현)
- 특정 금융 상품 가입 권유 ("OO은행 상품에 가입하세요" 같은 표현)

가능한 것:
- 재무 기초 습관 및 행동 제안
- 학습 순서와 개념 안내
- 계좌 종류 일반적 설명 (ISA, IRP, CMA 등 개념 설명)
- 단계별 목표 설정 및 완료 기준 제시
- 구체적이고 오늘 바로 실천 가능한 행동 안내

응답 형식 (순수 JSON만 반환, 코드블록 없이):
{
  "currentStatus": "현재 재무 상태 2~3문장 요약",
  "goalPath": "목표까지 방향과 예상 기간 1~2문장",
  "steps": [
    {
      "order": 1,
      "title": "단계 제목 (10자 이내)",
      "description": "이 단계에서 달성할 것 2~3문장. 구체적으로.",
      "why": "왜 이 단계가 중요한지 1~2문장. 건너뛰면 어떤 문제가 생기는지.",
      "actions": [
        "오늘 바로 할 수 있는 행동 (5분 이내)",
        "이번 주 안에 할 행동 (30분~1시간)",
        "이번 달 안에 완성할 행동 (꾸준히)"
      ],
      "checkPoints": [
        "이 단계 완료 기준 1 (측정 가능하게)",
        "이 단계 완료 기준 2"
      ],
      "nomingTip": "노밍의 한마디: 이 단계의 핵심 팁이나 자주 하는 실수 주의사항 1~2문장",
      "relatedBites": ["관련 경제한잎 검색 키워드1", "키워드2"],
      "estimatedDays": 30,
      "category": "기초 | 저축 | 투자 | 절세 | 부동산 | 연금"
    }
  ],
  "todayAction": "오늘 5분 안에 할 수 있는 행동 1가지",
  "warning": null
}

steps는 4~6개. actions는 정확히 3개. checkPoints는 2~3개. relatedBites는 1~3개 단어.
각 단계는 이전 단계 위에 자연스럽게 쌓이도록 순서를 설계할 것.`
...
재무 자립도 점수: ${independenceResult.score}/50점 (${independenceResult.label})`
  ...
  try {
    ...
    return {
      currentStatus: '재무 기초부터 차근차근 시작해요. 작은 습관 하나가 큰 변화를 만들어요.',
      goalPath: '6개월 안에 탄탄한 재무 기반을 만들어봐요.',
      steps: [
        {
          order: 1,
          title: '비상금 만들기',
          description: '생활비 3개월치 비상금을 파킹통장이나 CMA에 준비해요. 비상금은 투자가 아니라 보험이에요. 언제든 꺼낼 수 있는 곳에 두는 게 핵심이에요.',
          why: '비상금이 없으면 예상치 못한 지출에 고금리 대출을 쓰게 돼요. 비상금이 있어야 투자도 흔들리지 않아요.',
          actions: ['오늘 내 통장 잔액 확인하고 월 생활비 계산하기', '파킹통장 또는 CMA 계좌 개설 알아보기', '매월 고정 자동이체로 비상금 적립 설정하기'],
          checkPoints: ['생활비 3개월치 금액이 별도 통장에 모였다', '해당 통장은 투자나 소비에 쓰지 않는다'],
          nomingTip: '비상금 통장은 이름을 "비상금 — 절대 건드리지 말 것"으로 저장해두는 게 심리적으로 효과적이에요!',
          relatedBites: ['비상금', 'CMA'],
          estimatedDays: 90,
          category: '기초',
        },
      ],
