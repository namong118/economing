...
function highlightNumbers(text) {
  ...
  return parts.map((part, i) =>
    /^\d/.test(part)
      ? <span key={i} style={{ color: 'var(--c-amber-700)', fontWeight: 600 }}>{part}</span>
      : part
  );
}
...
export default function DailyBiteCard({ bite, hideButton }) {
  ...
  return (
    <div style={{
      background: '#fff', borderRadius: 12, border: '0.5px solid var(--c-line)',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 5 }}>
          ...
          <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, background: 'var(--c-green-100)', color: 'var(--c-forest-700)', border: '0.5px solid var(--c-line)' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 26, fontWeight: 700, color: 'var(--c-forest-700)', lineHeight: 1.2, marginBottom: 4 }}>
          {bite.title}
        </div>
        <div style={{ fontSize: 13, color: 'var(--c-forest-700)', lineHeight: 1.5 }}>
          {bite.summary}
        </div>
      </div>
      ...
      {/* 픽토그램 영역 */}
      <div style={{
        background: 'var(--c-canvas)', borderRadius: 10, border: '0.5px solid var(--c-line)',
      ...
      {/* 설명 */}
      <p style={{ fontSize: 13, color: 'var(--c-slate)', lineHeight: 1.6, marginBottom: bite.realLifeExample ? 14 : 0 }}>
        {bite.description}
      </p>
      ...
      {!hideButton && (
        <button
          ...
          style={{
            width: '100%', background: 'var(--c-green-500)', color: '#fff',
