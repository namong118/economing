  ...
  return (
    <div>
      ...
      <div style={{
        ...
        border: '1.5px solid #DCF5EB', borderRadius: '22px',
        padding: '22px', display: 'flex', alignItems: 'center', gap: '16px',
        marginBottom: '12px',
      }}>
        {profile?.avatar_url ? (
          <img src={profile.avatar_url} alt="프로필" style={{ width: '64px', height: '64px', borderRadius: '50%', objectFit: 'cover', border: '3px solid #fff', boxShadow: '0 4px 14px rgba(33,197,142,0.2)', flexShrink: 0 }} />
        ) : (
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'linear-gradient(135deg,#21C58E,#1AAD7D)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '26px', color: '#fff', fontWeight: '700', border: '3px solid #fff', boxShadow: '0 4px 14px rgba(33,197,142,0.2)', flexShrink: 0 }}>
            {(profile?.nickname || user.email || '?')[0].toUpperCase()}
          </div>
        )}
        <div style={{ flex: 1, minWidth: 0 }}>
          <h2 style={{ fontSize: '20px', fontWeight: '900', color: '#0F172A', letterSpacing: '-0.6px', marginBottom: '2px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div style={{ flex: 1, height: '5px', background: '#E2E8F0', borderRadius: '100px', overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: '100px', background: 'linear-gradient(90deg,#21C58E,#1AAD7D)', width: `${progressPercent}%`, transition: 'width 0.6s' }} />
            </div>
            {nextLevel && <span style={{ fontSize: '11px', color: '#94A3B8', fontWeight: '600', whiteSpace: 'nowrap' }}>{nextLevel.emoji} {xpNeeded} XP</span>}
          </div>
        </div>
      </div>

      {/* ── 2. 성장 단계 ── */}
      <div style={{ background: '#fff', borderRadius: '18px', border: '1.5px solid #E8F5EF', padding: '20px', marginBottom: '12px' }}>
        <p style={{ fontSize: '11px', fontWeight: '700', color: '#94A3B8', letterSpacing: '0.8px', marginBottom: '14px' }}>성장 단계</p>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
          <span style={{ fontSize: '32px' }}>{currentLevel.emoji}</span>
          <div>
            <p style={{ fontSize: '17px', fontWeight: '900', color: '#0F172A', letterSpacing: '-0.5px' }}>{currentLevel.label} 단계</p>
          ...
          {LEVELS.map((l, i) => {
            ...
            return (
              <div key={l.key} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                <div title={l.label} style={{ width: isCurrent ? '28px' : '21px', height: isCurrent ? '28px' : '21px', borderRadius: '50%', flexShrink: 0, background: isCurrent ? '#21C58E' : isPast ? '#DCF5EB' : '#F4FAF6', border: isCurrent ? '2px solid #fff' : isPast ? '1.5px solid #A7F3D0' : '1.5px solid #E2E8F0', boxShadow: isCurrent ? '0 0 0 3px rgba(33,197,142,0.2)' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: isCurrent ? '14px' : '10px' }}>
                  {l.emoji}
                </div>
                {i < LEVELS.length - 1 && <div style={{ flex: 1, height: '2px', margin: '0 2px', background: isPast ? '#21C58E' : '#E2E8F0', borderRadius: '2px' }} />}
              </div>
            );
          })}
        </div>
      ...
      {isOnboarded ? (
        <div style={{ background: '#fff', borderRadius: '18px', border: '1.5px solid #E8F5EF', padding: '20px', marginBottom: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
            <p style={{ fontSize: '11px', fontWeight: '700', color: '#94A3B8', letterSpacing: '0.8px' }}>경제 프로필</p>
            <button
              onClick={() => navigate('/onboarding')}
              style={{ background: 'none', border: 'none', fontSize: '12px', color: '#94A3B8', cursor: 'pointer', fontWeight: '600', padding: 0 }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {econInfo && (
              <div style={{ background: econInfo.bg, border: `1.5px solid ${econInfo.border}`, borderRadius: '12px', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <p style={{ fontSize: '11px', fontWeight: '700', color: '#94A3B8', marginBottom: '1px', letterSpacing: '0.4px' }}>경제 수준</p>
                  <p style={{ fontSize: '14px', fontWeight: '800', color: econInfo.color }}>{econInfo.label} <span style={{ fontSize: '12px', color: '#64748B', fontWeight: '500' }}>— {econInfo.desc}</span></p>
                </div>
              </div>
            )}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              {invInfo && (
                <div style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', borderRadius: '10px', padding: '10px 12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontSize: '18px' }}>{invInfo.emoji}</span>
                  <div>
                    <p style={{ fontSize: '10px', fontWeight: '700', color: '#94A3B8', marginBottom: '1px', letterSpacing: '0.4px' }}>투자 경험</p>
                    <p style={{ fontSize: '12px', fontWeight: '700', color: '#0F172A' }}>{invInfo.label}</p>
                  </div>
                </div>
              )}
              {occInfo && (
                <div style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', borderRadius: '10px', padding: '10px 12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <div>
                    <p style={{ fontSize: '10px', fontWeight: '700', color: '#94A3B8', marginBottom: '1px', letterSpacing: '0.4px' }}>현재 상황</p>
                    <p style={{ fontSize: '12px', fontWeight: '700', color: '#0F172A' }}>{occInfo.label}</p>
                  </div>
                </div>
              )}
            </div>
            {interests.length > 0 && (
              <div>
                <p style={{ fontSize: '11px', fontWeight: '700', color: '#94A3B8', letterSpacing: '0.6px', marginBottom: '8px' }}>관심 분야</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  {interests.map(tag => (
                    <span key={tag} style={{ fontSize: '12px', fontWeight: '700', color: '#21C58E', background: '#F4FAF6', border: '1.5px solid #DCF5EB', borderRadius: '100px', padding: '4px 12px' }}>{tag}</span>
                  ))}
                </div>
              </div>
            )}
        ...
        <div style={{ background: '#fff', borderRadius: '18px', border: '1.5px dashed #A7F3D0', padding: '28px', textAlign: 'center', marginBottom: '12px' }}>
          <img src={`${BASE_URL}coach.png`} alt="노밍" style={{ width: '48px', height: '48px', borderRadius: '13px', objectFit: 'cover', margin: '0 auto 10px', display: 'block' }} />
          <p style={{ fontSize: '15px', fontWeight: '800', color: '#0F172A', marginBottom: '5px' }}>경제 성장 진단을 완료해보세요</p>
          <p style={{ fontSize: '13px', color: '#64748B', lineHeight: '1.6', marginBottom: '16px' }}>온보딩을 완료하면 노밍이 맞춤 코칭을 시작해요.</p>
          <button onClick={() => navigate('/onboarding')} style={{ padding: '10px 24px', borderRadius: '12px', background: 'linear-gradient(135deg,#21C58E,#1AAD7D)', color: '#fff', border: 'none', fontSize: '14px', fontWeight: '800', cursor: 'pointer', boxShadow: '0 4px 14px rgba(33,197,142,0.3)' }}>
            온보딩 시작하기 →
          </button>
        </div>
      )}
      ...
      {/* ── 4. 노밍 분석 ── */}
      {isOnboarded && (
        <div style={{ background: 'linear-gradient(145deg,#FFFBEA,#FFF4CC)', border: '1.5px solid #FFE08A', borderRadius: '18px', padding: '18px 20px', display: 'flex', gap: '12px', alignItems: 'flex-start', marginBottom: '24px' }}>
          <div style={{ position: 'relative', flexShrink: 0 }}>
            <img src={`${BASE_URL}coach.png`} alt="노밍" style={{ width: '42px', height: '42px', borderRadius: '12px', objectFit: 'cover' }} />
            <div style={{ position: 'absolute', bottom: '-2px', right: '-2px', width: '11px', height: '11px', borderRadius: '50%', background: '#21C58E', border: '2px solid #FFFBEA' }} />
          </div>
          <div style={{ flex: 1 }}>
            ...
            <button onClick={() => navigate('/coach')} style={{ marginTop: '10px', padding: '7px 14px', borderRadius: '100px', background: '#21C58E', color: '#fff', border: 'none', fontSize: '12px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 2px 8px rgba(33,197,142,0.3)' }}>
              노밍에게 더 물어보기 →
            </button>
          </div>
        </div>
  ...
  return (
    <div>
      <div style={{ marginBottom: '20px' }}>
        <h2 style={{ fontSize: '22px', fontWeight: '900', color: '#0F172A', letterSpacing: '-0.7px', marginBottom: '3px' }}>
          📖 나만의 경제사전
        </h2>
        <p style={{ fontSize: '13px', color: '#64748B', fontWeight: '500' }}>
      ...
      {!loaded ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 20px', background: '#fff', borderRadius: '18px', border: '1.5px solid #E8F5EF' }}>
          ...
          <p style={{ fontSize: '15px', fontWeight: '800', color: '#0F172A', marginBottom: '6px', letterSpacing: '-0.3px' }}>
        ...
        <>
          <p style={{ fontSize: '12px', fontWeight: '700', color: '#94A3B8', letterSpacing: '0.4px', marginBottom: '12px' }}>
    ...
    <div>
      ...
      {econLevel && (
        <div style={{
          ...
          border: '1.5px solid #FFE08A', borderRadius: '14px',
            ...
            <div key={step.step} style={{ display: 'flex', gap: '12px' }}>
              ...
              <div
                style={{
                  ...
                  background: '#fff',
                  border: `1.5px solid ${isRecommended ? step.color + '50' : '#E8F5EF'}`,
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '10px' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: '14px', fontWeight: '800', color: '#0F172A', letterSpacing: '-0.3px', marginBottom: '2px' }}>
                      Step {step.step}. {step.title}
                    </p>
                    <p style={{ fontSize: '11px', color: '#94A3B8', fontWeight: '500' }}>{step.duration}</p>
                  </div>
                ...
                {isExpanded && (
                  <div style={{ marginTop: '12px' }}>
                    <p style={{ fontSize: '13px', color: '#475569', lineHeight: '1.75', marginBottom: '12px' }}>
                      {step.description}
                    </p>
                        ...
                        {step.coachQuestions.map(q => (
                          <button
                            ...
                            style={{
                              ...
                              borderRadius: '10px', fontSize: '12px', color: '#374151',
    ...
    return (
      <PageWrapper>
        <div style={{ padding: '80px 24px', textAlign: 'center' }}>
          ...
          <button onClick={() => navigate('/login')} style={{ padding: '12px 28px', borderRadius: '12px', background: 'linear-gradient(135deg,#21C58E,#1AAD7D)', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: '700', fontSize: '14px' }}>
            로그인하기
          </button>
        </div>
      </PageWrapper>
    ...
    <PageWrapper>
      <div style={{ background: '#F4FAF6', minHeight: 'calc(100vh - 64px)' }}>
        ...
        <div style={{ padding: '20px 20px 0', maxWidth: '720px', margin: '0 auto' }}>
          <h1 style={{ fontSize: '24px', fontWeight: '900', color: '#0F172A', letterSpacing: '-0.7px' }}>
            내 성장
          </h1>
        </div>
        ...
        <div style={{
          ...
          background: '#fff', borderBottom: '1.5px solid #E8F5EF',
          marginTop: '12px',
        }}>
              ...
              {TABS.map(tab => (
                <button
                  ...
                  style={{
                    ...
                    color: activeTab === tab.key ? '#21C58E' : '#94A3B8',
